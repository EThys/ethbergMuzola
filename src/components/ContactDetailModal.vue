      ...
      >
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            <div>
              <h2 class="text-2xl font-bold text-gray-900">Détails du contact</h2>
          ...
          <div class="p-6 space-y-6">
            ...
            <!-- Messages History -->
            <div v-if="contact.messages && Array.isArray(contact.messages) && contact.messages.length > 0">
              <label class="block text-sm font-medium text-gray-700 mb-2">Historique des messages</label>
                  ...
                  <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center space-x-2">
                      <div :class="[
                        ...
                        {{ message.sender_name?.charAt(0).toUpperCase() || 'A' }}
                      </div>
                      <div>
            ...
            <!-- Original Message -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Message initial</label>
            ...
            <div class="border-t border-gray-200 pt-4">
              ...
              <form @submit.prevent="sendReply" class="space-y-4">
                    ...
                    <label class="flex items-center space-x-3 p-3 border-2 border-gray-200 rounded-lg hover:border-green-500/30 cursor-pointer transition-all">
                      ...
                      <div class="flex items-center space-x-2 flex-1">
                        ...
                        <div>
                          ...
                          <p class="text-xs text-gray-500" v-if="contact.phone">Envoyer un message WhatsApp</p>
                ...
                <button
                  ...
                  class="w-full px-4 py-3 bg-primary text-white rounded-lg font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
                >
                  <svg v-if="isSending" class="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <span>{{ isSending ? 'Envoi en cours...' : 'Envoyer la réponse' }}</span>
                </button>
              </form>
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-200">
              <div class="flex items-center space-x-4">
                ...
                <span v-if="contact.type" class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-semibold">
                  {{ contact.type }}
                </span>
              </div>
...
const toggleStatus = async () => {
  try {
    const response = await fetch(`${apiUrl}/api/contact/${props.contact.id}`, {
      method: 'PATCH',
      headers: {
      ...
      body: JSON.stringify({
        is_read: !props.contact.is_read,
      }),
    });

    const data = await response.json();
    if (data.success) {
      props.contact.is_read = !props.contact.is_read;
      emit('updateStatus');
    }
  } catch (error) {
...
const sendReply = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/api/contact/${props.contact.id}/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
      ...
      body: JSON.stringify({
        ...
        recipient_email: props.contact.email,
        recipient_phone: props.contact.phone,
        recipient_name: props.contact.name,
      }),
    });

    const data = await response.json();
    if (data.success) {
      // Add the new message to the contact's messages array
      if (!props.contact.messages) {
        props.contact.messages = [];
      }
      if (data.data && data.data.message) {
        props.contact.messages.push(data.data.message);
      }
    ...
    console.error('Erreur:', error);
    alert(`Erreur lors de l'envoi de la réponse: ${error.message || 'Erreur de connexion'}`);
  } finally {
